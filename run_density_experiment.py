...
# ================================================================
# EXPERIMENT F — No-switch training steps sweep
# ================================================================

def run_experiment_f(
    data_dir_noswitch, device, pool_noswitch,
    n_instances=3, seed=0, wandb_run=None,
) -> pd.DataFrame:
    """
    Training steps sweep on single-regime (no switching) time series.

    Mirrors Experiment C but both training pool and evaluation datasets are
    generated without any regime switching.  Ablation to establish a baseline:
    how does performance scale with training data when switching is absent?

    Training:  no-switch pool (generate_pool.py --no_switch)
    Eval:      DATASETS_NOSWITCH from generated_data_noswitch/
               (generate_noswitch_data.py)
    """
    print("\n" + "="*60)
    print("EXPERIMENT F: No-switch training steps sweep")
    print(f"  n_instances={n_instances}  ar_coeff_scale=1.2")
    print(f"  pool_noswitch={pool_noswitch}")
    print(f"  data_dir_noswitch={data_dir_noswitch}")
    print("="*60)

    if pool_noswitch is None:
        raise ValueError(
            "Experiment F requires --pool_noswitch. "
            "Run: python generate_pool.py --no_switch --out pool_noswitch.npz"
        )

    missing = [
        ds for ds in DATASETS_NOSWITCH for i in range(n_instances)
        if not (Path(data_dir_noswitch) / f"{ds}_r{i}.npz").exists()
    ]
    if missing:
        print(f"[error] {len(missing)} no-switch dataset files missing in {data_dir_noswitch}. "
              f"Run: python generate_noswitch_data.py")
        return pd.DataFrame()
    ...
    val_monitor_ns = ["A1_ar2_coeffs_easy", "C1_arma21_coeffs_var",
                      "D1_arima211", "F1_seasonal_sarimax", "G1_exogenous_only"]
    val_loader = get_val_monitor_loader(
        data_dir_noswitch, context_len, val_frac, batch_size, val_monitor_ns)
    ...
    for steps in step_counts:
        ...
        sampler = build_sampler(
            ar_coeff_scale=1.2, seed=seed,
            ar_order_lo=2, ar_order_hi=2,
            pool_path=pool_noswitch,
            family_weights=FAMILY_PRESETS["full"],
        )

        results = train_and_eval(
            model, sampler, val_loader, steps, batch_size, lr, device,
            data_dir_noswitch, n_instances, context_len, val_frac, None,
            datasets=DATASETS_NOSWITCH,
            wandb_run=wandb_run, wandb_prefix=f"exp_f/steps_{steps}",
        )
        ...
        if wandb_run is not None:
            wandb_run.log({
                "exp_f/steps":        steps,
                "exp_f/total_series": total,
                "exp_f/mean_all":     results["mean_all"],
            })
...
def main():
    ...
    ap.add_argument("--data_dir_noswitch",  type=str, default="generated_data_noswitch",
                    help="Directory with no-switch eval datasets (Experiment F).")
    ap.add_argument("--pool_noswitch",      type=str, default=None,
                    help="No-switch training pool for Experiment F.")
    ...
    if "F" in args.experiments:
        df = run_experiment_f(
            data_dir_noswitch=args.data_dir_noswitch,
            device=device,
            pool_noswitch=args.pool_noswitch,
            n_instances=args.n_instances,
            seed=args.seed, wandb_run=wandb_run,
        )
        if not df.empty:
            df.to_csv("results_density_exp_f.csv", index=False)
            saved.append("results_density_exp_f.csv")
