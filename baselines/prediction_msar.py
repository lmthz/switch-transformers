...
def evaluate_msar_fixed_order(
    ...
    # --- in-sample (train) ---
    pred_train_all = predict_in_sample(res)  # typically length n_train - order
    y_train = y[:n_train]
    ...
    # --- out-of-sample (val) ---
    n_val = len(y) - n_train
    pred_val = predict_out_of_sample(res, y_train=y_train, steps=n_val)  # length n_val
    y_val = y[n_train:]
    ...
    # errors (aligned)
    err_train = y_train_aligned - pred_train
    err_val = y_val_aligned - pred_val
